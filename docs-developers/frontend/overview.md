...
| `/book/[libraryId]?connection=…&path=…` | `(app)/book/[libraryId].tsx` | Book detail: an overview (breadcrumbs, versions, cover hero, stats, play/resume + download control, and the capability-gated community-metadata **About** block) above a scrollable pill **tab row** - Chapters/Files, Recaps, Characters, Bookmarks, History, Notes, Series - whose meta-driven tabs appear only when the data exists ([State & data](state-and-data.md#the-book-screens-tabs)). Same flat query-param addressing as the library routes. |
